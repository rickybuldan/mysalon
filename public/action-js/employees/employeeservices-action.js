...
function getListData() {
    let dtpr = $("#table-list").DataTable({
        ajax: {
            url: baseUrl + "/ajax-listemployeeservices",
            type: "GET",
...
function editdata(rowData) {
    ...
    $("#form-employee").val(rowData.id_employee).trigger("change");
    $("#form-service").val(rowData.id_service).trigger("change");
...
$("#add-btn").on("click", function (e) {
    ...
    isObject = {};
    isObject["id"] = null;
    $("#form-employee").val("").trigger("change");
    $("#form-service").val("").trigger("change");
    $("#modal-data").modal("show");
});
...
function saveData() {
    ...
    if (isObject["id"] == null) {
        url = baseUrl + "/ajax-createemployeeservice";
    } else {
        url = baseUrl + "/ajax-updateemployeeservice";
    }
...
function checkValidation() {
    ...
    if (
        validationSwalFailed(
            (isObject["id_employee"] = $("#form-employee").val()),
            "Choose an employee."
        )
    )
        return false;
    if (
        validationSwalFailed(
            (isObject["id_service"] = $("#form-service").val()),
            "Choose a service."
        )
    )
        return false;
